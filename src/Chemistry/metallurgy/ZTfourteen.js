...
const clickHanlderSan = function handleClickSan(){
  ...
  if (stat==="Not Visited"){
   ...
   props.btnFourthirteen()
    ...
    <form>
    <p><i>Question No.14</i><br></br> If it is difficult to reduce metal oxides to the metal using carbon, the reduction is done using</p>
      <div>
        <input
          type="radio"
          value="a"
      ...
      <div>
        <input
          ...
          onClick={handleChange}
        /> Electrolytic method     </div>       
      
    </form><br></br>
    <div className={props.buttons}>
         
        <button onClick={()=>(clickHanlderNext)(navigate('/ZTfifteen'))}>Next</button>
        <button onClick={()=>(clickHanlderMfr)(navigate('/ZTfifteen'))}>Mark For Review</button>
        <button onClick={()=>(clickHanlderSan)(navigate('/ZTfifteen'))}>Save & Next</button>
        <button onClick={()=>(clickHanlderNext)(navigate('/ZTthirteen'))}>Previous</button>
...
export default connect(mapStateToProps,mapDispatchToProps) (ZTfourteen)