...
<div style={dStyle}><button style={bstyle} onClick={()=>(clickHandler)(navigate('/ZPscore'))}>Confirm</button>    <button style={bstyle1} onClick={()=>navigate('/ZPone')}>Cancel</button></div>
</div>
<p>{props.score}</p>
</div>
  )
...
const mapStateToProps=state=>{
    ...
    export default connect(mapStateToProps,mapDispatchToProps) (ZPconfirm)