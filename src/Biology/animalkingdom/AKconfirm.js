...
const clickHandler = function handleClickNext(){
    ...
    if(status[21]==="Answered"||status[20]==="Marked For Review"){
      if(opted[20]===correct[20]){
        props.scoreUpdate();
...
return (
    ...
    <h1 style ={hStyle}> <img  style={logoStyle} src={Logo} alt="Logo" /></h1> 
           ...
           <div>
          {/* <div style={divStyle}><p>Name Of the Candidate:{props.name}<br></br>
          Name Of the test:Diseases</p>
          <DYtimer /></div><br></br> */}
          <div><h2 style ={hstyle}>Dear {props.name}, Are you sure you want to submit for final marking?<br></br>
No changes will be allowed after submission.</h2></div>
<div style={dStyle}><button style={bstyle} onClick={()=>(clickHandler)(navigate('/AKscore'))}>Confirm</button>    <button style={bstyle1} onClick={()=>navigate('/AKone')}>Cancel</button></div>
</div>
{/* <p>{props.score}</p> */}
</div>
  )
...
const mapStateToProps=state=>{
    ...
    export default connect(mapStateToProps,mapDispatchToProps) (AKconfirm)