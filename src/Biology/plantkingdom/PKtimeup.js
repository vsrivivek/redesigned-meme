...
return (
    ...
    <h1 style ={hStyle}> <img  style={logoStyle} src={Logo} alt="Logo" /></h1> 
          ...
          <div><h2 style ={hstyle}>Dear {props.name}, The time is up!!! Click the button below to go the Score Page</h2></div>
<div style={dStyle}><button style={bstyle} onClick={()=>(clickHandler)(navigate('/PKscore'))}>Confirm</button></div>
</div>
<p>{props.score}</p>
</div>
  )
...
const mapStateToProps=state=>{
    ...
    export default connect(mapStateToProps,mapDispatchToProps) (PKtimeup)