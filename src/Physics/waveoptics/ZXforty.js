...
const clickHanlderSan = function handleClickSan(){
  ...
  if (stat==="Not Visited"){
   ...
   props.btnFourthirtynine()
    ...
    <form>
    <p><i>Question No.40</i><br></br> Consider a star emitting light of frequency 'f' as it moves either directly away from us or directly towards us.If the frequency of the light that we receive is 10% higher than the frequency of light it is emitting, the speed and direction of motion of the star respectively are (C is the speed of light) </p>
      <div>
        <input
          type="radio"
          value="a"
    ...
    <button onClick={()=>(clickHanlderNext)(navigate('/ZXthirtynine'))}>Previous</button>
        <button onClick={()=>(clickHanlderSan)(navigate('/ZXthirtynine'))}>Save & Previous</button>
        <button onClick={()=>clickHanlderSan()}>Save</button>
        <button onClick={()=>(clickHanlderMfr)(navigate('/ZXthirtynine'))}>Mark For Review</button>
        </div>
        </div> 
...
export default connect(mapStateToProps,mapDispatchToProps) (ZXforty)