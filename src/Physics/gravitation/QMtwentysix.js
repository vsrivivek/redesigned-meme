...
const clickHanlderSan = function handleClickSan(){
  ...
  if (stat==="Not Visited"){
   ...
   props.btnFourtwentyfive()
    ...
    <form>
    <p><i>Question No.26</i><br></br>   A body is projected vertically up from the surface of the earth, with a velocity mv<sub>e</sub> where m{'\u003c'}1 and v<sub>e</sub> is the escape velocity. If R is radius of earth, the maximum distance from the centre of the earth to which body raises is
</p>
      <div>
        <input
          type="radio"
    ...
    <div className={props.buttons}>
        ...
        <button onClick={()=>(clickHanlderNext)(navigate('/QMtwentyseven'))}>Next</button>
        <button onClick={()=>(clickHanlderMfr)(navigate('/QMtwentyseven'))}>Mark For Review</button>
        <button onClick={()=>(clickHanlderSan)(navigate('/QMtwentyseven'))}>Save & Next</button>
        <button onClick={()=>(clickHanlderNext)(navigate('/QMtwentyfive'))}>Previous</button>
...
export default connect(mapStateToProps,mapDispatchToProps) (QMtwentysix)
