...
const clickHanlderSan = function handleClickSan(){
  ...
  if (stat==="Not Visited"){
   ...
   props.btnFourtwentyfour()
    ...
    <form>
    <p><i>Question No.25</i><br></br>  If M is mass of the earth and R is the radius, then the velocity with which a body should be projected from the surface of the earth such that it just reaches a maximum height equal to 'R' above the surface is
       </p>
      <div>
        <input
          type="radio"
    ...
    <div className={props.buttons}>
        ...
        <button onClick={()=>(clickHanlderNext)(navigate('/QMtwentysix'))}>Next</button>
        <button onClick={()=>(clickHanlderMfr)(navigate('/QMtwentysix'))}>Mark For Review</button>
        <button onClick={()=>(clickHanlderSan)(navigate('/QMtwentysix'))}>Save & Next</button>
        <button onClick={()=>(clickHanlderNext)(navigate('/QMtwentyfour'))}>Previous</button>
...
export default connect(mapStateToProps,mapDispatchToProps) (QMtwentyfive)