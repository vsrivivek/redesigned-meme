...
<div style={dStyle}><button style={bstyle} onClick={()=>(clickHandler)(navigate('/QMscore'))}>Confirm</button>    <button style={bstyle1} onClick={()=>navigate('/QMone')}>Cancel</button></div>
</div>
<p>{props.score}</p>
</div>
  )
...
const mapStateToProps=state=>{
    ...
    export default connect(mapStateToProps,mapDispatchToProps) (QMconfirm)