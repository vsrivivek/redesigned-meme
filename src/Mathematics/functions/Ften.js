...
const clickHanlderSan = function handleClickSan(){
  ...
  if (stat==="Not Visited"){
   ...
   props.btnFournine()
    ...
    <form>
    <p><i>Question No.10</i><br></br> If f(x)= log[(1-x)/1+x)]    then f[(x{'\u2081'}+x{'\u2082'})/(1+x{'\u2081'}x{'\u2082'})] 
    ...
    <div className={props.buttons}>
        ...
        <button onClick={()=>(clickHanlderNext)(navigate('/Feleven'))}>Next</button>
        <button onClick={()=>(clickHanlderMfr)(navigate('/Feleven'))}>Mark For Review</button>
        <button onClick={()=>(clickHanlderSan)(navigate('/Feleven'))}>Save & Next</button>
        <button onClick={()=>(clickHanlderNext)(navigate('/Fnine'))}>Previous</button>
...
export default connect(mapStateToProps,mapDispatchToProps) (Ften)