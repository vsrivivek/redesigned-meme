...
const clickHanlderSan = function handleClickSan(){
  ...
  if (stat==="Not Visited"){
   ...
   props.btnFourthirtyseven()
    ...
    <form>
    <p><i>Question No.38</i><br></br> If f: [0,∞){'\u2192'}[0,∞) is defined by f(x)= x/(1+x), then f is
</p>
      <div>
        <input
          type="radio"
    ...
    <div className={props.buttons}>
        ...
        <button onClick={()=>(clickHanlderNext)(navigate('/Fthirtynine'))}>Next</button>
        <button onClick={()=>(clickHanlderMfr)(navigate('/Fthirtynine'))}>Mark For Review</button>
        <button onClick={()=>(clickHanlderSan)(navigate('/Fthirtynine'))}>Save & Next</button>
        <button onClick={()=>(clickHanlderNext)(navigate('/Fthirtyseven'))}>Previous</button>
...
export default connect(mapStateToProps,mapDispatchToProps) (Fthirtyeight)