...
const clickHanlderSan = function handleClickSan(){
  ...
  if (stat==="Not Visited"){
   ...
   props.btnFourfifteen()
    ...
    <form>
    <p><i>Question No.16</i><br></br> If f : R{'\u2192'}R is defined by f(x) = [x/5] for x ∈ R, where [y] denotes the greatest integer not exceeding y, then {`{f(x) : |x| < 71}`} =
    ...
    <div className={props.buttons}>
        ...
        <button onClick={()=>(clickHanlderNext)(navigate('/Fseventeen'))}>Next</button>
        <button onClick={()=>(clickHanlderMfr)(navigate('/Fseventeen'))}>Mark For Review</button>
        <button onClick={()=>(clickHanlderSan)(navigate('/Fseventeen'))}>Save & Next</button>
        <button onClick={()=>(clickHanlderNext)(navigate('/Ffifteen'))}>Previous</button>
...
export default connect(mapStateToProps,mapDispatchToProps) (Fsixteen)